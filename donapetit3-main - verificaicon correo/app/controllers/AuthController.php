...
public function enviarCodigo(): void {
    ...
    $emailEnviado = $emailService->enviarCodigoRecuperacion(
        $resultado['email'],
        $resultado['nombre'],
        $resultado['codigo']
    );
    ...
    if (!$emailEnviado) {
        $_SESSION['error'] = 'No se pudo enviar el email. Intentá nuevamente.';
        header('Location: ?controller=Auth&action=mostrarRecuperacion');
        exit;
    }
    ...
    $_SESSION['success'] = 'Te enviamos un código de verificación a <strong>' . htmlspecialchars($email) . '</strong>. Revisá tu bandeja de entrada y carpeta de spam.';
