...
class AuthController {
    ...
    public function enviarCodigo(): void {
        ...
        try {
            // Usar AuthService para generar el código
            $authService = new AuthService();
            $resultado = $authService->requestPasswordReset($email);

            $codigo = $resultado['codigo'];
            $usuario = $resultado['usuario'];

            // Intentar enviar el email con el código
            try {
                $emailService = new EmailService();
                $enviado = $emailService->enviarCodigoRecuperacion(
                    $usuario['email'],
                ...
                if (!$enviado) {
                    $_SESSION['error'] = 'No pudimos enviar el email. Verifica la configuración SMTP.';
                    header('Location: ?controller=Auth&action=mostrarRecuperacion');
                    exit;
                }
                ...
                unset($_SESSION['recuperacion_codigo']); // Limpiar código previo
                $_SESSION['success'] = 'Te enviamos un código de recuperación a tu email. <br><small>Válido por 15 minutos.</small>';
            ...
            // Errores de validación o usuario no encontrado
