...

{
    ...
    public function productosDisponibles(): void
    {
        ...
        $userRole = normalize_role($usuario['rol'] ?? null);
    ...
    public function solicitar(): void
    {
        ...
        if ($fechaProgramada === '') {
            ...
            if (!$fecha || $fecha->format('Y-m-d') !== $fechaProgramada) {
                $errores[] = 'La fecha programada no es valida.';
            }
        }
        ...
        try {
            ...
            $_SESSION['success'] = 'Solicitud enviada exitosamente. El donante la revisara pronto.';
        } catch (\Exception $e) {
            error_log('Error al crear solicitud: ' . $e->getMessage());
            $_SESSION['error'] = 'No se pudo procesar la solicitud. Intenta mas tarde.';
        }
    ...
    public function confirmar(): void
    {
        ...
        if ($idSolicitudDetalle <= 0) {
            $_SESSION['error'] = 'Solicitud no valida.';
            $this->redirect('?controller=Producto&action=misProductos');
            return;
        }
        ...
        if ($resultado) {
            $_SESSION['success'] = 'Solicitud confirmada y stock actualizado.';
        } else {
    ...
    public function rechazar(): void
    {
        ...
        $_SESSION['success'] = $resultado ? 'Solicitud rechazada.' : 'No se pudo rechazar la solicitud.';
     ...
     * la f%%rmula de Haversine si tienes las coordenadas del receptor
...

    {
        ...
        // TODO: Implementar c%lculo real de distancia usando coordenadas del usuario
