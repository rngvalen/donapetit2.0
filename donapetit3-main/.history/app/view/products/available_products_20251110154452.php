...
foreach ($productos as $p) {
    ...
    if (!isset($porDonante[$donante])) {
        $porDonante[$donante] = [];
    }
    $porDonante[$donante][] = $p;
}
?>
<section class="max-w-7xl mx-auto px-4 py-8">
  ...
  <?php else: ?>
    ...
    <!-- Agrupar por donante -->
    <?php foreach ($porDonante as $nombreDonante => $productosDelDonante): ?>
      <div class="mb-10 bg-white rounded-3xl shadow-lg border border-slate-200 p-6">
        ...
        <div class="flex items-center justify-between mb-6 pb-4 border-b-2 border-blue-500">
          <div>
            <h2 class="text-2xl font-bold text-slate-800">
              🏪 <?= htmlspecialchars($nombreDonante) ?>
            </h2>
            <p class="text-sm text-slate-500 mt-1">
              <?= count($productosDelDonante) ?> producto(s) disponible(s)
            </p>
          </div>
          
          <!-- Botón para solicitar productos de este donante -->
          <a href="?controller=Solicitud&action=crear&id_donante=<?= htmlspecialchars($nombreDonante) ?>"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <?php foreach ($productosDelDonante as $p): ?>
