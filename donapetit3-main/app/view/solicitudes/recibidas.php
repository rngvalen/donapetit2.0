          ...
          <div class="space-y-2 text-sm text-slate-600 mb-4">
            ...
            <?php if ($isAprobadaSinConfirmar && $tiempoRestante): ?>
              <div class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p class="font-bold text-blue-900">⏰ Tiempo para retiro: <?= $tiempoRestante ?></p>
                <p class="text-xs text-blue-700 mt-1">
                  Límite: <?= date('d/m/Y H:i', strtotime($s['fecha_limite_retiro'])) ?>
                </p>
              </div>
            <?php endif; ?>
            ...
            <?php if ($isAprobadaConfirmada && isset($s['fecha_retiro'])): ?>
              <div class="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p class="font-bold text-green-900">✓ Retiro confirmado</p>
          ...
          <div class="pt-4 border-t border-slate-200 flex gap-2">
            ...
            <?php if ($s['estado'] === 'Pendiente'): ?>
              <a href="?controller=Solicitud&action=aprobar&id=<?= $s['id_solicitud'] ?>"
                 onclick="return confirm('¿Aprobar y RESERVAR el stock por 2 horas? El receptor tendrá ese tiempo para retirar.')"
            ...
            <?php if ($isAprobadaSinConfirmar): ?>
              <a href="?controller=Solicitud&action=confirmarRetiro&id=<?= $s['id_solicitud'] ?>"
                 ...
                 class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg transition text-sm shadow-lg">
                ✓ Confirmar Retiro
              </a>
            <?php endif; ?>
          </div>
