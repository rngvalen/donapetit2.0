<?php
$user = htmlspecialchars($userName ?? 'Usuario', ENT_QUOTES, 'UTF-8');

// Obtener resumen de solicitudes
$resumen = ['Pendiente' => 0, 'Aprobada' => 0, 'Rechazada' => 0, 'total' => 0];
if (isset($_SESSION['user']) && $_SESSION['user']['rol'] === 'receptor') {
    try {
        require_once __DIR__ . '/../../model/Solicitud.php';
        $solicitudModel = new Solicitud();
        $resumen = $solicitudModel->obtenerResumenReceptor((int)$_SESSION['user']['id']);
    } catch (\Throwable $e) {
        error_log("Error al obtener resumen: " . $e->getMessage());
    }
}

$cards = [
    [
        'title' => 'Encargar productos',
        'description' => 'Solicita donaciones disponibles de productos que necesites.',
        'href' => 'index.php?controller=Producto&action=productosDisponibles',
        'cta' => 'Ver disponibles',
        'icon' => '🛒',
        'badge' => null,
        'highlight' => false,
    ],
    [
        'title' => 'Mis solicitudes',
        'description' => 'Consulta el estado de tus pedidos y gestiona reservas.',
        'href' => 'index.php?controller=Solicitud&action=missolicitudes',
        'cta' => 'Ver solicitudes',
        'icon' => '📋',
        'badge' => $resumen['Pendiente'] > 0 ? $resumen['Pendiente'] : null,
        'highlight' => $resumen['Aprobada'] > 0,
    ],
    [
        'title' => 'Estadísticas',
        'description' => 'Consulta métricas sobre tus solicitudes, productos más recibidos y frecuencia mensual.',
        'href' => 'index.php?controller=Home&action=statics',
        'cta' => 'Ver dashboard',
        'icon' => '📊',
        'badge' => null,
        'highlight' => false,
    ],
    [
        'title' => 'Mapa de donantes',
        'description' => 'Explora negocios cercanos con donaciones disponibles en un mapa interactivo.',
        'href' => 'index.php?controller=Map&action=index',
        'cta' => 'Abrir mapa',
        'icon' => '🗺️',
        'badge' => null,
        'highlight' => false,
    ],
];
?>
<section class="py-10 text-center">
  <h1 class="text-3xl font-extrabold text-slate-900">
    Bienvenido <span class="italic text-brand/90"><?php echo $user; ?></span>
  </h1>
  <p class="mt-3 text-slate-600">Encuentra y solicita donaciones de alimentos disponibles.</p>
  
  <!-- Alertas de notificaciones -->
  <?php if ($resumen['Aprobada'] > 0): ?>
    <div class="mt-6 mx-auto max-w-2xl">
      <div class="rounded-xl border-2 border-green-400 bg-green-50 p-4 shadow-lg">
        <div class="flex items-center justify-center gap-3">
          <div class="flex-shrink-0">
            <svg class="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
          </div>
          <div class="flex-1 text-left">
            <p class="text-sm font-bold text-green-900">
              ¡<?= $resumen['Aprobada'] ?> solicitud<?= $resumen['Aprobada'] > 1 ? 'es' : '' ?> aprobada<?= $resumen['Aprobada'] > 1 ? 's' : '' ?>!
            </p>
            <p class="text-xs text-green-700">
              Tus pedidos fueron aprobados y están listos para retirar
            </p>
          </div>
          <a href="index.php?controller=Solicitud&action=missolicitudes"
             class="flex-shrink-0 rounded-lg bg-green-600 hover:bg-green-700 px-4 py-2 text-sm font-semibold text-white transition">
            Ver detalles
          </a>
        </div>
      </div>
    </div>
  <?php endif; ?>

  <?php if ($resumen['Rechazada'] > 0): ?>
    <div class="mt-4 mx-auto max-w-2xl">
      <div class="rounded-xl border-2 border-red-300 bg-red-50 p-4 shadow-lg">
        <div class="flex items-center justify-center gap-3">
          <div class="flex-shrink-0">
            <svg class="w-8 h-8 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                    d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
          </div>
          <div class="flex-1 text-left">
            <p class="text-sm font-bold text-red-900">
              <?= $resumen['Rechazada'] ?> solicitud<?= $resumen['Rechazada'] > 1 ? 'es' : '' ?> rechazada<?= $resumen['Rechazada'] > 1 ? 's' : '' ?>
            </p>
            <p class="text-xs text-red-700">
              Algunas solicitudes no pudieron ser procesadas
            </p>
          </div>
          <a href="index.php?controller=Solicitud&action=missolicitudes"
             class="flex-shrink-0 rounded-lg bg-red-600 hover:bg-red-700 px-4 py-2 text-sm font-semibold text-white transition">
            Ver detalles
          </a>
        </div>
      </div>
    </div>
  <?php endif; ?>

  <?php if ($resumen['Pendiente'] > 0): ?>
    <div class="mt-4 mx-auto max-w-2xl">
      <div class="rounded-xl border-2 border-blue-300 bg-blue-50 p-4">
        <p class="text-sm text-blue-800">
          📋 Tienes <strong><?= $resumen['Pendiente'] ?></strong> solicitud<?= $resumen['Pendiente'] > 1 ? 'es' : '' ?> en espera de respuesta
        </p>
      </div>
    </div>
  <?php endif; ?>
</section>

<section class="mx-auto grid max-w-6xl gap-6 py-8 sm:grid-cols-2 lg:grid-cols-4">
  <?php foreach ($cards as $card): ?>
    <?php
    $title = htmlspecialchars($card['title'], ENT_QUOTES, 'UTF-8');
    $description = htmlspecialchars($card['description'], ENT_QUOTES, 'UTF-8');
    $href = htmlspecialchars($card['href'], ENT_QUOTES, 'UTF-8');
    $cta = htmlspecialchars($card['cta'], ENT_QUOTES, 'UTF-8');
    $icon = $card['icon'] ?? '';
    $badge = $card['badge'] ?? null;
    $highlight = $card['highlight'] ?? false;
    
    $cardClass = $highlight 
      ? 'border-2 border-green-400 bg-green-50 shadow-green-200' 
      : 'border border-slate-200 bg-white';
    
    $buttonClass = $highlight
      ? 'bg-green-600 hover:bg-green-700 focus-visible:ring-green-600'
      : 'bg-brand hover:bg-brand/90 focus-visible:ring-brand/80';
    ?>
    <article class="relative flex h-full flex-col justify-between rounded-2xl <?= $cardClass ?> p-6 text-left shadow-sm transition hover:shadow-md">
      
      <?php if ($badge !== null): ?>
        <span class="absolute -top-3 -right-3 flex h-10 w-10">
          <span class="absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75 animate-ping"></span>
          <span class="relative inline-flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-sm font-bold text-white shadow-lg">
            <?= $badge > 9 ? '9+' : $badge ?>
          </span>
        </span>
      <?php endif; ?>

      <div>
        <div class="flex items-center gap-2 mb-2">
          <?php if ($icon): ?>
            <span class="text-2xl"><?= $icon ?></span>
          <?php endif; ?>
          <h2 class="text-lg font-semibold text-slate-900"><?php echo $title; ?></h2>
        </div>
        <p class="mt-2 text-sm text-slate-600"><?php echo $description; ?></p>
      </div>
      
      <a href="<?php echo $href; ?>"
         class="mt-6 inline-flex items-center justify-center gap-2 rounded-lg <?= $buttonClass ?> px-3 py-2 text-sm font-medium text-white focus:outline-none focus-visible:ring-2">
        <?php echo $cta; ?>
      </a>
    </article>
  <?php endforeach; ?>
</section>

<style>
  @keyframes ping {
    75%, 100% {
      transform: scale(1.5);
      opacity: 0;
    }
  }
  .animate-ping {
    animation: ping 1.5s cubic-bezier(0, 0, 0.2, 1) infinite;
  }
</style>